...
export function useLocalHighScore(name: string) {
  const [highScore, setHighScore] = useState<number>(0);

  useEffect(() => {
    // Only access localStorage in the browser
    const savedHighScore = localStorage.getItem(name);
    if (savedHighScore) {
  ...
  const updateHighScore = (newScore: number) => {
    ...
    localStorage.setItem(name, newScore.toString()); // Save the new high score to localStorage
  };
